...
class Book(db.Model):
  ...
  title = db.Column(db.String(), nullable=False) #unique = True to make each book one type title
  author = db.Column(db.String(), nullable=False) #nullable is to make item not empty
...
def get_all_books():
  ...
  # marshmallow does the same thing from lines 59 to 67
  return jsonify(book_schema.dump(all_books))
